...
class SheetManager {
    ...
    // Función para obtener datos de usuario por código de 4 dígitos
    async getUserByCode(code: string, phoneNumber: string): Promise<{name: string, email: string, denuncia: string, code: string, date: string} | null> {
        try {
            const result = await this.sheets.spreadsheets.values.get({
                spreadsheetId: this.spreadsheetId,
                range: 'Visitors!A:F', // Columns A-F (phone, name, email, denuncia, code, date)
            });

            const rows = result.data.values;
            if (rows) {
                // Find row where column E (code) matches the provided code AND column A (phone) matches phoneNumber
                const userRow = rows.find(row => row[4] === code && row[0] === phoneNumber);
    ...
    async userExists(number: string): Promise<boolean> {
        try {
            const result = await this.sheets.spreadsheets.values.get({
                spreadsheetId: this.spreadsheetId,
                range: 'Visitors!A:A', // Asumiendo que los números de teléfono están en la columna A
            });
    ...
    async createUser(number: string, name: string): Promise<void> {
        try {
            // currentDate is now passed as a parameter
            // Agregar el usuario a la pestaña 'Users' (ahora con denuncia, código aleatorio en columna E y fecha en columna F)
    ...
    // Function to get visitor name from 'Visitors' sheet by phone number
    async getVisitorName(number: string): Promise<string | null> {
        try {
            const result = await this.sheets.spreadsheets.values.get({
                spreadsheetId: this.spreadsheetId,
                range: 'Visitors!A:B',
            ...
            console.error("Error al obtener el nombre del visitante in getVisitorName:", error);
            return null;
        }
    }
}
